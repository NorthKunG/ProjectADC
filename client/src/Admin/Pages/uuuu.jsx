    ...
    <div className="col-span-2 flex items-center gap-3">
      ...
      <label className="font-semibold text-lg cursor-pointer">เป็น SPEC ICT</label>
        ...
        <div className="flex flex-col items-center">
          <div className="w-32 h-32 border flex items-center justify-center text-3xl text-blue-600 mb-4">+</div>
          <div className="flex gap-4">
            <div className="w-24 h-24 border flex items-center justify-center text-2xl text-blue-600">+</div>
            <div className="w-24 h-24 border flex items-center justify-center text-2xl text-blue-600">+</div>
            <div className="w-24 h-24 border flex items-center justify-center text-2xl text-blue-600">+</div>
          </div>
        </div>
  ...
  {product.specs.map((spec, index) => (
    <div key={index} className="flex items-center gap-2 mb-2">
      ...
      {index === product.specs.length - 1 && (
        <button onClick={addSpecField} className="px-4 py-2 bg-blue-500 text-white rounded-lg">+</button>
      )}
      {/* ปุ่มลบ (-) แสดงทุกแถว แต่ต้องมีอย่างน้อย 1 รายการ */}
      {product.specs.length > 1 && (
        <button onClick={() => removeSpecField(index)} className="px-4 py-2 bg-red-500 text-white rounded-lg">-</button>
      )}
    </div>
  ))}
</div>
...
<div className="col-span-2 sm:col-span-1">
  ...
  <div className="relative max-w-xs"> {/* ปรับขนาดให้ไม่กว้างเกิน */}
    <input 
      type="number" 
      name="price" 
      value={product.price} 
...
<div className="flex flex-wrap gap-4">
  {[
    { label: "มีสินค้า", color: "text-green-600 bg-green-100 border-green-400" },
    { label: "สินค้าหมด", color: "text-red-600 bg-red-100 border-red-400" },
    { label: "สินค้าใหม่", color: "text-yellow-600 bg-yellow-100 border-yellow-400" },
    { label: "โปรโมชั่น", color: "text-blue-600 bg-blue-100 border-blue-400" },
  ].map(({ label, color }) => (
    <label key={label} className={`flex items-center gap-2 px-4 py-2 rounded-full border ${color} cursor-pointer`}>
      <input type="radio" name="status" value={label} checked={product.status === label} onChange={handleChange} className="hidden" />
      <div className="w-4 h-4 border-2 border-gray-400 rounded-full flex items-center justify-center">
        {product.status === label && <div className="w-2 h-2 bg-blue-600   rounded-full"></div>}
      </div>
